...
(function(){
  ...
  const CATS=['コンセント','スイッチ','情報','分電盤','コメント','換気扇'];
  const LS='kairo_user_symbols_v3';
  ...
  CATS.forEach(cat=>{
    ...
    const sel=document.createElement('select');sel.className='bg-transparent text-xs w-20';sel.dataset.cat=cat;
    const updateSel=()=>{sel.innerHTML='<option value="">-</option>';Object.keys(symbols[cat]).forEach(id=>{const o=document.createElement('option');o.value=id;o.textContent=symbols[cat][id].name;sel.appendChild(o);});sel.innerHTML+='<option value="__new__">＋登録</option>';};
    updateSel();
    sel.onchange=()=>{
  ...
  svg.addEventListener('mousedown', (e)=>{
    ...
    nodes.push({id:nextId++, type:'symbol', x,y, cat:currentSymbol.cat, sid:currentSymbol.id, rotation:0, selected:false});
    rebuild(); save(); draw();
  }, true);
  ...
  window.draw=function(){
    ...
    nodes.forEach(n=>{const g=document.createElementNS(NS,'g');const rot=n.rotation||0;g.setAttribute('transform',`translate(${n.x},${n.y}) rotate(${rot})`);g.style.cursor=mode==='move'?'move':'pointer';if(n.selected)g.setAttribute('filter','url(#glow)');
      ...
      else if(n.type==='symbol'){const def=symbols[n.cat]?.[n.sid];if(def){const wrapper=document.createElementNS(NS,'g');wrapper.innerHTML=def.svg; // SVG断片をそのまま
        // 選択時は枠を付ける
        if(n.selected){const box=document.createElementNS(NS,'rect');box.setAttribute('x','-14');box.setAttribute('y','-14');box.setAttribute('width','28');box.setAttribute('height','28');box.setAttribute('fill','none');box.setAttribute('stroke','#f97316');box.setAttribute('stroke-width','2');box.setAttribute('stroke-dasharray','3 3');g.appendChild(box);}
  ...
  draw();
  console.log('kairo-symbols v3: プルダウン登録式');
})();
